...
- **The browser demo's CPU half is a port**: the field schedule, temporal weights, vertical taps and clock are a hand port of `Model.cpp`, `Schedule.cpp`, `Controls.cpp` and `Clock.cpp`, and nothing checks it. Change any of those and change `demo/plugin.js` by hand.
...
`demo/` is the page at **standards-demo.stoatworks-labs.com**, deployed from
...
`~/Projects/infrastructure/stoatworks-backend/resolume-demo/sync.sh standards` and is not
