...
/**
    ...
    **It reallocates only when it has to.** Ensure() is called every frame and
    ...
    host's picture changes width or the standards change height. A
    reallocation clears, so the field store is carried across a width change
    by `Standards::resizeStore` first (see `sttest --resize`).
    ...
    **It owns its filtering.** Every buffer in this plugin is lines and
    fields read texel for texel with `texelFetch` -- two adjacent rows of a
    frame are two different moments, and a filtered read across them is a
    read across time -- so every buffer here is Nearest. Linear and
    Mipmapped are tinsel's and are unused.

    Copied from tinsel (by way of rebate), where it was written for a glow
