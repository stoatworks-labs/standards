...
test card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/sttest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card has to move.** Temporal and Motion Comp are about what happens
between fields; on a still picture every temporal aperture returns the same
field and there is no motion to compensate, and both would read as dead.
`sttest --out` renders the moving card, 40 frames at 60 fps, which is past the
converter's two-field latency and the priming.

**The conversion controls need a conversion.** Vertical Taps and Softness are
only the identity at Direction = Same with no pre-filter; the default is
625/50 > 525/59.94, so they sweep live on the defaults. Nothing here needs a
CONTEXT entry today; a new control that does must say what else has to be
true, in CONTEXT.

**An option's range is its element count.** An option parameter's range reads
back 0..1 whatever its element count, so `sttest --list` prints the count as
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "sttest"))
    ...
    scratch = tempfile.mkdtemp(prefix="stsweep")

    # One render first: a runner with no GL at all cannot sweep anything, and
    # that has to read as a skip, loudly, not as eight dead controls.
